...
TEST_CASE("IRIView") {
    ...
    CHECK_NOTHROW(IRIView{"htt-p://example/path?query#frag"}.quick_validate());
    CHECK_NOTHROW(IRIView{"htt-p:foo/bar"}.quick_validate());
    CHECK_NOTHROW(IRIView{"htt-p:#frag"}.quick_validate());
    CHECK_THROWS_WITH_AS(IRIView{"example"}.quick_validate(), doctest::Contains("unexpected relative"), InvalidIRI);
    CHECK_THROWS_WITH_AS(IRIView{"htt?p://example"}.quick_validate(), doctest::Contains("invalid scheme"), InvalidIRI);
    CHECK_NOTHROW(IRIView{"http://user@example:123"}.quick_validate());
    CHECK_THROWS_WITH_AS(IRIView{"http://user@exa@mple:123"}.quick_validate(), doctest::Contains("invalid host"), InvalidIRI);
    CHECK_THROWS_WITH_AS(IRIView{"http://us]er@example:123"}.quick_validate(), doctest::Contains("invalid userinfo"), InvalidIRI);
    CHECK_NOTHROW(IRIView{"http://us:er@example:123"}.quick_validate());
    CHECK_THROWS_WITH_AS(IRIView{"http://user@example:12a3"}.quick_validate(), doctest::Contains("invalid port"), InvalidIRI);
    CHECK_THROWS_WITH_AS(IRIView{"htt-p://example/pat[h"}.quick_validate(), doctest::Contains("invalid path"), InvalidIRI);
    CHECK_THROWS_WITH_AS(IRIView{"htt-p://example/path?que]ry#frag"}.quick_validate(), doctest::Contains("invalid query"), InvalidIRI);
    CHECK_THROWS_WITH_AS(IRIView{"htt-p://example/path?query#fra]g"}.quick_validate(), doctest::Contains("invalid fragment"), InvalidIRI);
    CHECK_NOTHROW(IRIView{"htt-p://exa\U0001f34cmple/pa\U0001f34cth?que\U0001f34cry#fra\U0001f34cg"}.quick_validate());
    CHECK_THROWS_WITH_AS(IRIView{"htt\U0001f34cp://example"}.quick_validate(), doctest::Contains("invalid scheme"), InvalidIRI);
    CHECK_NOTHROW(IRIView{"http://[FEDC:BA98:7654:3210:FEDC:BA98:7654:3210]:80/index.html"}.quick_validate());
...
TEST_CASE("base") {
    ...
    // normal
    CHECK(fact.from_relative("g:h").identifier() == "g:h");
    CHECK(fact.from_relative("g").identifier() == "http://a/b/c/g");
    CHECK(fact.from_relative("./g").identifier() == "http://a/b/c/g");
    CHECK(fact.from_relative("g/").identifier() == "http://a/b/c/g/");
    CHECK(fact.from_relative("/g").identifier() == "http://a/g");
    CHECK(fact.from_relative("//g").identifier() == "http://g");
    CHECK(fact.from_relative("?y").identifier() == "http://a/b/c/d;p?y");
    CHECK(fact.from_relative("g?y").identifier() == "http://a/b/c/g?y");
    CHECK(fact.from_relative("#s").identifier() == "http://a/b/c/d;p?q#s");
    CHECK(fact.from_relative("g#s").identifier() == "http://a/b/c/g#s");
    CHECK(fact.from_relative("g?y#s").identifier() == "http://a/b/c/g?y#s");
    CHECK(fact.from_relative(";x").identifier() == "http://a/b/c/;x");
    CHECK(fact.from_relative("g;x").identifier() == "http://a/b/c/g;x");
    CHECK(fact.from_relative("g;x?y#s").identifier() == "http://a/b/c/g;x?y#s");
    CHECK(fact.from_relative("").identifier() == "http://a/b/c/d;p?q");
    CHECK(fact.from_relative(".").identifier() == "http://a/b/c/");
    CHECK(fact.from_relative("./").identifier() == "http://a/b/c/");
    CHECK(fact.from_relative("..").identifier() == "http://a/b/");
    CHECK(fact.from_relative("../").identifier() == "http://a/b/");
    CHECK(fact.from_relative("../g").identifier() == "http://a/b/g");
    CHECK(fact.from_relative("../..").identifier() == "http://a/");
    CHECK(fact.from_relative("../../").identifier() == "http://a/");
    CHECK(fact.from_relative("../../g").identifier() == "http://a/g");
    ...
    CHECK(fact.from_relative("./../g").identifier() == "http://a/b/g");
    CHECK(fact.from_relative("./g/.").identifier() == "http://a/b/c/g/");
    CHECK(fact.from_relative("g/./h").identifier() == "http://a/b/c/g/h");
    CHECK(fact.from_relative("g/../h").identifier() == "http://a/b/c/h");
    CHECK(fact.from_relative("g;x=1/./y").identifier() == "http://a/b/c/g;x=1/y");
    CHECK(fact.from_relative("g;x=1/../y").identifier() == "http://a/b/c/y");
    // unexpected dots
    CHECK(fact.from_relative("g?y/./x").identifier() == "http://a/b/c/g?y/./x");
    CHECK(fact.from_relative("g?y/../x").identifier() == "http://a/b/c/g?y/../x");
    CHECK(fact.from_relative("g#y/./x").identifier() == "http://a/b/c/g#y/./x");
    CHECK(fact.from_relative("g#y/../x").identifier() == "http://a/b/c/g#y/../x");

    CHECK(fact.from_relative("http://ex.com/a/./b/../c").identifier() == "http://ex.com/a/c");
    CHECK(fact.from_maybe_relative("http://ex.com/a/./b/../c").identifier() == "http://ex.com/a/./b/../c");
    CHECK(fact.from_maybe_relative("/x/./b/../c").identifier() == "http://a/x/c");

    CHECK_NOTHROW(fact.set_base("tag:example"));
    CHECK(fact.from_relative("a").identifier() == "tag:a");
}

TEST_CASE("base reassign") {
    IRIFactory fact{"http://example/foo"};

    CHECK(fact.from_relative("bar").identifier() == "http://example/bar");

    CHECK(fact.get_base() == "http://example/foo");

    CHECK_THROWS_WITH_AS(fact.set_base("/bar"), doctest::Contains("unexpected relative"), InvalidIRI);

    CHECK(fact.get_base() == "http://example/foo");

    CHECK_NOTHROW(fact.set_base("http://example/bar"));

    CHECK(fact.get_base() == "http://example/bar");

    CHECK(fact.from_relative("foo").identifier() == "http://example/foo");
}

TEST_CASE("prefix") {
    IRIFactory fact{};

    CHECK_THROWS_WITH_AS(fact.assign_prefix(" ", "foo"), doctest::Contains("invalid prefix"), InvalidIRI);
    CHECK_THROWS_WITH_AS(fact.assign_prefix("abc.", "foo"), doctest::Contains("invalid prefix"), InvalidIRI);
    CHECK_THROWS_WITH_AS(fact.assign_prefix(".abc", "foo"), doctest::Contains("invalid prefix"), InvalidIRI);



    CHECK_NOTHROW(fact.assign_prefix("pre", "http://ex.org/"));
    CHECK_NOTHROW(fact.assign_prefix("foo", "http://foo.org/"));

    CHECK(fact.from_prefix("pre", "bar").identifier() == "http://ex.org/bar");
    CHECK(fact.from_prefix("foo", "bar").identifier() == "http://foo.org/bar");
    CHECK_THROWS_WITH_AS((void) fact.from_prefix("bar", "bar"), doctest::Contains("unknown prefix"), InvalidIRI);
    CHECK(fact.from_prefix("pre", "a/./b/../c").identifier() == "http://ex.org/a/./b/../c");

    CHECK_NOTHROW(fact.assign_prefix("pre", "http://ex.org/pre2/"));
    CHECK(fact.from_prefix("pre", "bar").identifier() == "http://ex.org/pre2/bar");

    CHECK_NOTHROW(fact.assign_prefix("dot", "http://ex.org/a/./b/../c#"));
    CHECK(fact.from_prefix("dot", "bar").identifier() == "http://ex.org/a/./b/../c#bar");

    fact.clear_prefix("pre");
    CHECK_THROWS_WITH_AS((void) fact.from_prefix("pre", "bar"), doctest::Contains("unknown prefix"), InvalidIRI);
...
TEST_CASE("relative prefix") {
    ...
    CHECK(fact.from_prefix("pre", "bar").identifier() == "http://ex.org/pre/bar");
    CHECK(fact.from_prefix("foo", "bar").identifier() == "http://ex.org/foo/bar");
    CHECK_THROWS_WITH_AS((void) fact.from_prefix("bar", "bar"), doctest::Contains("unknown prefix"), InvalidIRI);

    fact.assign_prefix_unchecked("pre", "pre2/");
    CHECK(fact.from_prefix("pre", "bar").identifier() == "http://ex.org/pre2/bar");

    CHECK(fact.from_prefix("pre", "a/./b/../c").identifier() == "http://ex.org/pre2/a/c");
    CHECK_NOTHROW(fact.assign_prefix("dot", "a/./b/../c#"));
    CHECK(fact.from_prefix("dot", "bar").identifier() == "http://ex.org/a/c#bar");
