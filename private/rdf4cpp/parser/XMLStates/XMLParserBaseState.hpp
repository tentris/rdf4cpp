...
namespace rdf4cpp::parser::xml_states {
    ...
    struct BaseState {  // NOLINT(*-special-member-functions)
        ...
        [[nodiscard]] virtual StateTransition on_characters(XMLOutputQueue &out, std::string_view chars, XMLStateInfo const &info) = 0;
        [[nodiscard]] virtual StateTransition on_start_element(XMLOutputQueue &out, std::string_view local_name, std::string_view uri, std::span<XMLAttribute> attributes, XMLStateInfo const &info) = 0;
        [[nodiscard]] virtual StateTransition on_end_element(XMLOutputQueue &out, XMLStateInfo const &info) = 0;
        ...
        static InheritedAttributeInfo get_inherited_attributes(XMLOutputQueue &out, std::span<XMLAttribute> attributes, XMLStateInfo const &info);
    };
}  // namespace rdf4cpp::parser::xml_states
