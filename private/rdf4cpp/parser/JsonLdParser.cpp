...
namespace rdf4cpp::parser {
    ...
    nonstd::expected<Literal, IStreamQuadIterator::error_type> IStreamQuadIterator::ImplJsonLd::make_literal(json_ld::LiteralMapping const &t) {
        try {
            return t.language.has_value() ? Literal::make_lang_tagged(t.value, *t.language) : Literal::make_simple(t.value);
        }
            ...
            { // 21
                ...
                if (c != simdjson::NO_SUCH_FIELD) {
                    ...
                    if (t != simdjson::ondemand::json_type::array && t != simdjson::ondemand::json_type::object) {
                        return make_error(ParsingError::Type::BadSyntax, "invalid scoped context, free floating scalar");
                    }
    ...
    json_ld::StringifyResult IStreamQuadIterator::ImplJsonLd::stringify(simdjson::ondemand::value v, bool normalize, bool force_double, bool escape_string) {
        ...
        if (t == simdjson::ondemand::json_type::number) {
            ...
            if (std::floor(d) != d || std::abs(d) >= 10.0e21 || force_double) {
                if (normalize) {
    ...
    nonstd::expected<json_ld::ExpandedLevel, IStreamQuadIterator::error_type> IStreamQuadIterator::ImplJsonLd::expand_level(json_ld::Context const &active_context,
        ...
        {
            elem_object.reset();
            auto r = expand_level_nested_recursive(result, elem_object, *active_ctx, *type_scoped_ctx, {}, active_property, input_type, base_iri, false);
            if (r.has_value()) {
        ...
        for (auto kv : elem_obj) {
            ...
            if (expanded_property->type == json_ld::IRIMappingType::Keyword) {
                ...
                // 13.4.13
                    ...
                    auto e = expand_level_nested_recursive(result, v, active_ctx, type_scoped_context, p, active_property, input_type, base_iri, !reverse);
                    if (e.has_value()) {
                        return e;
                    }
                }
                // 13.4.14 + 14
                    ...
                    for (ValueArrayIter iter{v}; iter != iter.end(); ++iter) {
                        ...
                        p.keys.emplace_back(std::in_place_type<std::string>, k);
                        iter.push_index(p);
                        auto e = expand_level_nested_recursive(result, *iter, active_ctx, type_scoped_context, p, active_property, input_type, base_iri, reverse);
                        if (e.has_value()) {
                            return e;
                        }
                    }
    ...
    simdjson::ondemand::value IStreamQuadIterator::ImplJsonLd::ValueArrayIter::operator*() {
        return std::visit([]<typename T>(T & t) -> simdjson::ondemand::value {
