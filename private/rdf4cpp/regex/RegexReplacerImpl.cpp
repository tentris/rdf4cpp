...
void RegexReplacer::Impl::regex_replace(std::string &str) const {
    ...
    auto rep = [&] {
        outsize = r.size();
        return pcre2_substitute_8(regex->compiled_regex.get(), reinterpret_cast<PCRE2_SPTR8>(str.data()), str.size(), 0, opt, nullptr, nullptr, reinterpret_cast<PCRE2_SPTR8>(rewrite.data()), rewrite.size(), reinterpret_cast<PCRE2_UCHAR8 *>(r.data()), &outsize);
    };
    auto e = rep();
